...
setwd("/Users/lordvillanelle/Desktop/data/")
...
#saveRDS(LAM1_WT.combined, "LAM1.rds")
#Can view object
LAM1_WT.combined[[]]
...
DimPlot(LAM1_WT.combined, reduction = "umap",label = TRUE,split.by = "orig.ident")
...
priorknowledgePaths<-c("MTOR signaling","MAP kinase","Protein kinase","MAPK")
priorknowledgeMOA<-c("CDK inhibitor","MTOR inhibitor","MEK inhibitor")
tablecellclounts <-table(LAM1_WT.combined@meta.data$nCount_RNA, LAM1_WT.combined@meta.data$celltype.orig.ident)
tablecellclounts<-as.data.frame(colSums(tablecellclounts))

new.cluster.idsmatchesMOA<-c()
new.cluster.idsmatchesPATHS<-c()
#i_indents<-10
#Scans each cell type and matches results with prior knowledge supplied by user
for(i_indents in 1:length(new.cluster.ids) ){
  print(new.cluster.ids[i_indents])
  if(paste(new.cluster.ids[i_indents],"_","LAM1",sep = "") %in% unique(Idents(LAM1_WT.combined)) && paste(new.cluster.ids[i_indents],"_","WT",sep = "") %in% unique(Idents(LAM1_WT.combined))){
    indexcountsLAM<-which(rownames(tablecellclounts)==paste(new.cluster.ids[i_indents],"_","LAM1",sep = ""))
    indexcountsWT<-which(rownames(tablecellclounts)==paste(new.cluster.ids[i_indents],"_","WT",sep = ""))
    if(tablecellclounts[indexcountsWT,] > 3 && tablecellclounts[indexcountsLAM,] > 3){
      LAM_WT.markersSM <- FindMarkers(LAM1_WT.combined, ident.1 = paste(new.cluster.ids[i_indents],"_","LAM1",sep = ""), ident.2 = paste(new.cluster.ids[i_indents],"_","WT",sep = ""), verbose = FALSE)
      ...
      #using threshold 200 includes sirolimus in the most sig drugs
      ...
      if(length(enrichedDown) !=0 && length(enrichedUp) !=0){
        ...
        pathsSM<-pathsSM[which(pathsSM$P.value<0.05),]
        pathsSMGO<-enrichedPaths[["GO_Molecular_Function_2015"]]
        pathsSMGO<-pathsSMGO[which(pathsSMGO$P.value<0.05),]
        
        matchKEGG<-unique (grep(paste(priorknowledgePaths,collapse="|"), 
                                pathsSM[,1], value=TRUE,ignore.case = T))
        
        matchGO<-unique (grep(paste(priorknowledgePaths,collapse="|"), 
                              pathsSMGO[,1], value=TRUE,ignore.case = T))
        
        if(length(matchKEGG) != 0 || length(matchGO) !=0){
          new.cluster.idsmatchesPATHS<-rbind(new.cluster.idsmatchesPATHS,cbind(new.cluster.ids[i_indents],"1"))
        }else{
          new.cluster.idsmatchesPATHS<-rbind(new.cluster.idsmatchesPATHS,cbind(new.cluster.ids[i_indents],"0"))
        }
        ...
        if(length(drugsUp) > 50){
          drugsUp<-drugsUp[1:50]
        }
        
        if(length(drugsDown) > 50){
          drugsDown<-drugsDown[1:50]
        }
        ...
        if(length(matchMOA) != 0 && length(which(c(1,2,3,4,5,6)%in%matchMOAIndexes)) !=0){
          #if(length(matchMOA) != 0){
          new.cluster.idsmatchesMOA<-rbind(new.cluster.idsmatchesMOA,cbind(new.cluster.ids[i_indents],"1"))
        }else{
          new.cluster.idsmatchesMOA<-rbind(new.cluster.idsmatchesMOA,cbind(new.cluster.ids[i_indents],"0"))
        }
...
print(new.cluster.idsmatchesPATHS)
print(new.cluster.idsmatchesMOA)
...
for(d in 1:length(AlldrugsBulk)){
  indexfoundd<-grep(AlldrugsBulk[d],drugInfo$pert_iname)
  if(!length(indexfoundd)==0){
    foundds<-foundds+1
    #MOA<-c(MOA,drugInfo$moa[indexfoundd])
    MOA<-rbind(MOA,cbind(drugInfo$pert_iname[indexfoundd],drugInfo$moa[indexfoundd],drugInfo$disease_area[indexfoundd]))
